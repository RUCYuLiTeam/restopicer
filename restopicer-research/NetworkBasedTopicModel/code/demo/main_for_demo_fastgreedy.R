...
topic_term <- getTopicMemberBipartiteMatrix(community_member_list = communities(fc),weight = "binary")
...
# network of topic
plotTopicNetworkReport(filename = "demo_FASTGREEDY_keyword",graph = coterm_g,community_member_list = communities(fc),showNamesInPlot = TRUE,plotCommunity = TRUE,plotOverallTopics = TRUE,path = "output/demo_FASTGREEDY_keyword/topic_term")
# transpose = FALSE
