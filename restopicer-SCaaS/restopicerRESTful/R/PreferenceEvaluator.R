elasticNetPreferenceEval <- function(enetmodel,predict_new_docs){
  ...
  #fits <- predict(object = enetmodel, newx = predict_new_docs, s = 0.5, type = "fit",mode = "fraction")
  #fits$fit
  fits <- predict(object = enetmodel,newx = predict_new_docs,s=0.5,type = "link",exact = T)
  fits
}
